...
class Captcha {
    ...
    */
    public static function reCaptcha(){
        $reCaptcha = Extension::where('slug', 'google-recaptcha2')->where('status', Status::ENABLE)->first();
    ...
    */
    public static function customCaptcha($width = '100%', $height = 46, $bgColor = '#003'){
        ...
        $captcha = Extension::where('slug', 'custom-captcha')->where('status', Status::ENABLE)->first();
        if (!$captcha) {
            return 0;
        }
    ...
    */
    public static function verifyGoogleCaptcha(){
        $pass = true;
        $googleCaptcha = Extension::where('slug', 'google-recaptcha2')->where('status', Status::ENABLE)->first();
        if ($googleCaptcha) {
    ...
    */
    public static function verifyCustomCaptcha(){
        $pass = true;
        $customCaptcha = Extension::where('slug', 'custom-captcha')->where('status', Status::ENABLE)->first();
        if ($customCaptcha) {
